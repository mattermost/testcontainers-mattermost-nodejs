...
var MattermostContainer = class {
  constructor() {
    ...
    this.start = () => __async(this, null, function* () {
      this.network = yield new Network().start();
      this.pgContainer = yield new PostgreSqlContainer("docker.io/postgres:15.2-alpine").withExposedPorts(5432).withDatabase("mattermost_test").withUsername("user").withPassword("pass").withNetworkMode(this.network.getName()).withWaitStrategy(Wait.forLogMessage("database system is ready to accept connections")).withNetworkAliases("db").start();
      this.container = yield new GenericContainer(defaultMattermostImage).withEnvironment(this.envs).withExposedPorts(8065).withNetwork(this.network).withNetworkAliases("mattermost").withCommand(this.command).withWaitStrategy(Wait.forLogMessage("Server is listening on")).withCopyFilesToContainer(this.configFile).withLogConsumer((stream) => {
        stream.on("data", (data) => {
...
export {
  MattermostContainer as default
};
